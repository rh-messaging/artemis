...
. JavaDocs are created correctly, including the diagrams (in `artemis-website/target/reports/apidocs/`)
...
Once the website is updated then send an email to `announce@apache.org`, `users@artemis.apache.org` & `dev@artemis.apache.org` with a subject like:
