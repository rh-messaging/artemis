...
[WARNING]
====
Server discovery is *disabled by default* for security reasons.

Before enabling server discovery, check that the broadcast endpoint authenticates the senders:

* *JGroups*: configure the JGroups `AUTH` protocol and/or TLS in the stack referenced by `jgroups-file` so only authorized members can send broadcasts.
* *UDP*: has no built-in sender authentication; use only on trusted networks.

Set the JVM system property `artemis.discovery.enabled=true` or the environment
variable `ARTEMIS_DISCOVERY_ENABLED=true` to enable discovery on clients and servers.

You can set the JVM system property for the broker by including
`-Dartemis.discovery.enabled=true` within the `JAVA_ARGS` within `etc/artemis.profile` and
`etc/artemis-utility.profile` for your broker instance.`

For clustered instances created with the CLI that require server discovery, you must pass
the hidden `--discovery-enabled` flag to `artemis create`.
The `--discovery-enabled` flag adds `-Dartemis.discovery.enabled=true` to `JAVA_ARGS` in
both profile files.
Use this only after reviewing the security requirements above.

See xref:threat-model.adoc#threat-model[threat model] for trust boundary implications.
====
...
When creating connections between nodes of a cluster to form a cluster connection, the broker uses a cluster user and cluster password which is defined in `broker.xml` (shown with the default values):
...
Every broker in the cluster must use matching values for `cluster-user` and `cluster-password`.

[IMPORTANT]
====
For safety, any connection using these credentials will be rejected.
*In order for brokers to cluster together properly these values must be changed from their defaults.*
====
