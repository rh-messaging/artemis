...
[IMPORTANT]
====
Review the xref:versions.adoc#version-history[version] history for specific steps required for your upgrade before proceeding.

If you are using journal replication for high availability, follow the xref:ha.adoc#rolling-upgrade[rolling upgrade procedure] to upgrade without downtime.
====
...
Please refer to the aforementioned xref:versions.adoc#version-history[versions] document for additional upgrade steps (if required).
