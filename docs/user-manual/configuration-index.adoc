...
| xref:persistence.adoc#configuring-the-message-journal[journal-lock-monitor-timeout]
| how long (in ms) between journal lock health checks.
| 2000

| xref:persistence.adoc#configuring-the-message-journal[journal-lock-monitor-max-retries]
| how many consecutive failed lock health checks to tolerate before declaring the lock lost.
| 0
